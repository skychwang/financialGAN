        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            with tf.variable_scope("Decoder"):
                ...
                with tf.variable_scope("rho"):
                    dec_rho = tf.nn.tanh(linear(dec_hidden, self.n_x))
...
class VRNN():
    def __init__(self, args, sample=False):
        ...
        def get_lossfunc(enc_mu, enc_sigma, dec_mu, dec_sigma, dec_rho, prior_mu, prior_sigma, y):
            ...
            return tf.reduce_mean(kl_loss + tf.losses.mean_squared_error(y,dec_rho))
        ...
        lossfunc = get_lossfunc(enc_mu, enc_sigma, dec_mu, dec_sigma, dec_rho, prior_mu, prior_sigma, flat_target_data[:,:args.chunk_samples])
        self.sigma = dec_sigma
        self.mu = dec_mu
    ...
    def sample(self, sess, args, num=4410, start=None):
        ...
        for i in range(num):
            ...
            #next_x = np.hstack((o_rho,2.*(o_rho > np.random.random(o_rho.shape[:2]))-1.))
            chunks[i] = next_x
            mus[i] = o_mu
            sigmas[i] = o_sigma
