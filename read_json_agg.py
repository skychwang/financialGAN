...
def read_multiple_days_data():
    ...
    for i in range(len(raw_orders)):
        ...
        tgt_path = os.path.join('NPY_1/'+raw_orders[i].replace('.json','.npy'))
...
def reshape_data(buy_sell_array, zero_one=True, history=100,order_stream=10,step_size=10,batch_size=32):
...
def aggregate_multi_days_data(zero_one=True, history = 100, order_stream=10,step_size=10, batch_size=32):
