...
class lstm_cond_gan(object):
        ...
        for i in range(train_steps):
            ...
            for j in range(100): # critic trained 100 times
                ...
                d_loss = self.model_truth.train_on_batch([history,history_full,noise_1,\
                    truth], [positive_y,negative_y,dummy_y])

            #Train Generator
            a_loss = self.model_fake.train_on_batch([history,history_full,noise_1], negative_y)

            #Logging
            log_mesg = "%d: [D_fake loss: %f,D_truth loss: %f, Total loss: %f] "\
                % (i, d_loss[0],d_loss[1],-(d_loss[0]+d_loss[1]))
            log_mesg = "%s  [A loss: %f]" % (log_mesg, a_loss)
            with open('log_goog_no_cda_new.txt','a') as f:
                f.write(log_mesg+'\n')
                f.close()
            if i % 1000 == 0:
    ...
    def predict(self,save_path='predict_no_cda_new.npy',length=600000,step_size=1,num_runs=1):

        #Load Data
        data = np.load(self.data_path, mmap_mode='r')
        #Load Generator
        gen = load_model('gnr_no_cda_new_20000')
